...
| No-speech 오류 후 멈춤 | 바람 소리 등으로 인식이 시작됐지만 실제 음성이 없어 `no-speech` 오류로 종료된 뒤, 다음 세션이 자동으로 재시작되지 않아 마이크가 반응하지 않음 | `recognition.onerror`에서 `no-speech`일 때 `pendingRecognitionStartRef`를 `true`로 설정해 재시작을 큐잉하고, `onend`에서 해당 플래그를 확인해 즉시 `startListening()`을 다시 호출하도록 변경. 동시에 `recognitionActiveRef`를 false로 초기화해 상태를 확실히 복원 |
| 장시간 구동 후 `SpeechRecognition.start()` 예외 반복 | 전시장에서 수 시간 뒤 `InvalidStateError`가 빈번히 발생하며 볼륨이 충분해도 인식이 재개되지 않음 | `RECOGNITION_RESTART_DELAY_MS`, `RECOGNITION_MAX_RETRIES`를 도입해 백오프 재시작 큐(`requestRecognitionRestart`)를 구현하고, `startListening()`에서 예외 발생 시 즉시 `recognitionActiveRef`를 false로 돌린 뒤 재시도를 예약 |
| 치명적 Recognition 오류 이후 영구 중단 | `network`, `service-not-allowed`, `audio-capture` 등 오류가 한 번 발생하면 이후 이벤트가 전혀 오지 않음 | 치명 오류 집합을 정의해 `recognition.onerror`에서 감지 시 기존 인스턴스를 폐기(`recognitionRef.current = null`)하고, 재시도 루프가 새 인스턴스를 생성하도록 변경 |
| `rendering/clearing` 상태가 고착되어 idle 전환 불가 | 캔버스 애니메이션 지연이나 background 탭 전환 후 `status`가 `rendering`에 머물러 볼륨 모니터링이 재시작되지 않음 | `RENDER_IDLE_WATCHDOG_MS` 기준 워치독을 추가해 비-idle 상태가 일정 시간 지속되면 강제로 `setStatus("idle")`를 실행하고, 대기 중이던 `startListening()`을 즉시 처리 |
| 마이크 스트림/AudioContext가 중간에 종료 | 전원/케이블 문제나 macOS 절전 후 `MediaStreamTrack.readyState === "ended"`가 되어 볼륨이 0만 반환 | `reinitializeAudioMonitoring()`을 분리해 트랙 `onended` 이벤트와 AudioContext `state`를 감시, 이상 시 스트림과 `AnalyserNode`를 모두 폐기 후 재생성. `AUDIO_HEALTH_CHECK_INTERVAL_MS` 간격으로 주기적 헬스 체크를 추가해 suspend 상태를 자동 resume |
